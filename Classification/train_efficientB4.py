...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=100, device='cuda', patience=30):
    model.to(device)
    best_loss = float('inf')
...
def main():
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=1e-3, epochs=100, steps_per_epoch=len(dataloaders['train']))

    # Train the classifier
    model = train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=100, device=device, patience=30)

    # Unfreeze all layers
    for param in model.parameters():
        param.requires_grad = True

    # Reset optimizer and scheduler for full model training
    optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-2)
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=1e-4, epochs=100, steps_per_epoch=len(dataloaders['train']))

    # Train the full model
    model = train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=100, device=device, patience=30)

    # Save the trained model
    torch.save(model.state_dict(), 'refined_advanced_torch_classification_model.pth')
